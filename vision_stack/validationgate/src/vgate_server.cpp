#include <validationgate/vgate_server.h>
#define PI 3.1414
#include <resources/topicHeader.h>

Vgate::Vgate(std::string name) : _it(_n), _s(_n, name, boost::bind(&Vgate::executCB, this, _1), false), _actionName(name)
{
    _sub = _it.subscribe(topics::CAMERA_BOTTOM_VGATE_IMAGE, 1, &Vgate::imageCallBack, this);
    ...
    switch(_goal->order)
    {
        ...
        case ALIGN_VGATE:
        {
            while(ros::ok())
            {
                ...
                {
                    _result.sequence.push_back(VGATE_ALIGNED);
                    break;
                }
...
{
    ros::init(argc, argv, "vgate_server");
    Vgate _vgateserver("vgate");
